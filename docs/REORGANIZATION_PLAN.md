...
- [x] Фаза 3.2: Сравнительное тестирование
- [ ] Фаза 3.3: Интеграционные тесты на Ground Truth
- [ ] Фаза 3.4: Аварийный анализ и исправление системы
- [ ] Фаза 3.5: Расширение тестирования по видению
- [ ] Фаза 4.1: README безопасность
- [ ] Фаза 4.2: Аудит конфигурации

## Цель

Привести кодовую базу в соответствие с утверждённой системной архитектурой (ADR-001 — ADR-015) и обеспечить 100% качество обработки чеков согласно PROJECT_VISION.
...
### 3.4 Аварийный анализ и исправление системы

**Статус:** В ПРОЦЕССЕ (🔴 ВЫСОЧАЙШИЙ ПРИОРИТЕТ)

**Принцип:** Ground Truth — неприкосновенный эталон. Все проваленные тесты — критические баги в системе.

**План действий:**

| Чек | Расхождение | Статус | Ответственный | Дедлайн |
|-----|-------------|--------|---------------|---------|
| IMG_1352 | +37.02 EUR | 🔴 КРИТИЧЕСКИЙ | Lead Developer | 24 часа |
| PL_001 | +98.57 PLN | 🔴 КРИТИЧЕСКИЙ | PL Locale Expert | 48 часов |
| PT_001 | +0.99 EUR | 🟡 ВЫСОКИЙ | Core Team | 72 часа |

**Методология отладки:**

1. **Пошаговый прогон через этапы пайплайна** с подробным логированием
2. **Визуальная отладка** с помощью нового скрипта `scripts/debug_pipeline.py`
3. **RCA (Root Cause Analysis)** для каждого проваленного checksum

**Новые инструменты:**
- [ ] `scripts/debug_pipeline.py` — визуализация работы пайплайна
- [ ] `scripts/step_by_step_debug.py` — пошаговое выполнение с выводом промежуточных данных
- [ ] `logs/pipeline_debug/` — директория для логов отладки

**Правила:**
- Никто не имеет права изменять Ground Truth данные
- Каждый проваленный тест = системный баг, требующий исправления
- Все исправления должны быть системными (уровень паттернов, не конкретных чеков)

---

### 3.5 Расширение тестирования по видению

**Статус:** ПЛАНИРУЕТСЯ

**Проблема:** Текущие тесты проверяют только checksum, но видение требует 100% качество по всем метрикам:
1. Точное количество товаров (0 расхождений)
2. Точные названия товаров (1 в 1)
3. Точные цены (0.00 расхождений)

**Решение:** Расширить тестовую инфраструктуру:

```python
# Новые метрики тестирования в scripts/test_vision_metrics.py
test_metrics = {
...
**Новые скрипты:**
- [ ] `scripts/test_vision_metrics.py` — запуск всех метрик видения
- [ ] `scripts/generate_validation_report.py` — детальный отчёт по расхождениям
- [ ] `tests/vision/test_zero_tolerance.py` — тесты на нулевую толерантность

**Метрики качества:**
- [ ] 100% чеков проходят checksum (±0.05)
- [ ] 100% чеков имеют точное количество товаров
- [ ] 95%+ чеков имеют точные названия товаров (similarity > 0.9)
- [ ] 100% чеков имеют точные цены товаров
...
- [ ] Создать `config/constants.py` для констант из ADR

---

## 🔴 ВАЖНОЕ ПРАВИЛО: НЕПРИКОСНОВЕННОСТЬ GROUND TRUTH

**Ground Truth в `docs/ground_truth/` — абсолютный эталон, проверенный и утверждённый автором проекта.**
...
|------|--------|-----------|--------|
| **3.4** | **Аварийный анализ и исправление системы** | 🔴🔴 КРИТИЧЕСКИЙ | IN PROGRESS |
| 3.4.1 | Отладка IMG_1352 (diff: 37.02) | 🔴🔴 | TODO |
| 3.4.2 | Отладка PL_001 (diff: 98.57) | 🔴🔴 | TODO |
| 3.4.3 | Отладка PT_001 (diff: 0.99) | 🔴 | TODO |
| **3.5** | **Расширение тестирования по видению** | 🔴 КРИТИЧЕСКИЙ | TODO |
| 1.1 | Контракты на Pydantic | ✅ | DONE |
| 1.2 | Интеграция контрактов | ✅ | DONE |
| 2.1 | Структура stages/ | ✅ | DONE |
| 2.2 | Реализация этапов | ✅ | DONE |
| 2.3 | Синхронизация документации | ✅ | DONE |
| 3.1 | Интеграция со скриптами | ✅ | DONE |
| 3.2 | Checksum валидация | ✅ | DONE |
| 3.3 | Сравнительное тестирование | ✅ | DONE |
| 4.1 | README безопасность | 🟢 | TODO |
| 4.2 | Аудит конфигурации | 🟢 | TODO |
...
1. ✅ **0 FAILED тестов** — все Ground Truth чеки проходят checksum (±0.05)
2. ✅ **Все метрики видения** проверяются автоматически (количество товаров, имена, цены)
3. ✅ **Ground Truth остается неприкосновенным** (ни одного изменения)
4. ✅ **Системные решения** задокументированы для каждого исправления
5. ✅ **Каждый проваленный тест** имеет RCA и исправление
6. ✅ **Контракты на Pydantic** с валидацией
7. ✅ **Пайплайн D2** = 6 этапов (Layout→Locale→Store→Metadata→Items→Validation)

---

## Связанные документы

- [PROJECT_VISION.md](PROJECT_VISION.md) — фундаментальные принципы (100% качество, неприкосновенность Ground Truth)
...
- [ADR-009: Гарантии качества D2](architecture/decisions/009_d2_quality_guarantees.md)