...
## ВАЖНО: Что такое Ground Truth

**Ground Truth — это ЭТАЛОННЫЕ данные, созданные ЧЕЛОВЕКОМ.**

```
Ground Truth ≠ результат работы D1
Ground Truth = ручная работа человека, проверенная на 100%
```

### Алгоритм работы:

```
1. ЧЕЛОВЕК смотрит ФИЗИЧЕСКИЙ ЧЕК
   ↓
2. ЧЕЛОВЕК создаёт Ground Truth (эталон, 100% верно)
   ↓
3. D1 обрабатывает ИЗОБРАЖЕНИЕ чека → raw_ocr_results.json
   ↓
4. ТЕСТИРОВАНИЕ: сравниваем D1 Output vs Ground Truth
   ↓
5. Находим ошибки OCR
```
...
| Принцип | Объяснение |
|---------|------------|
| **D1 НЕ трогает Ground Truth** | D1 только читает изображение и выдаёт OCR результат |
| **Ground Truth = входные данные для тестов** | Используется для валидации качества D1 |
| **Человек = гарант качества** | Только человек может создать и утвердить Ground Truth |
| **100% точность обязательна** | Ground Truth должен соответствовать физическому чеку на 100% |

---

**Назначение документа:** Пошаговая инструкция для создания Ground Truth при получении нового изображения чека.
