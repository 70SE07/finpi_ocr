...
1. **Language-agnostic:** D1 не знает ничего о языках и локалях.
   Google Vision сам определяет язык текста.
   Определение локали — задача D2, не D1.

2. **Content-agnostic:** D1 не знает что на изображении (чек, меню, документ).
   Его задача — просто распознать текст.

3. **Locale-agnostic:** D1 не передаёт language hints в Google Vision API.
   Это было бы нарушением SRP — D1 отвечает за OCR, не за локализацию.
...
5. **SRP** - D1 language-agnostic, D2 отвечает за локали