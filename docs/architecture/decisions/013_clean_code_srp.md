...
#### D1: Pre-OCR Operations

| Operation | Файл | ЦКП | НЕ отвечает за |
|-----------|------|-----|----------------|
...
### 2. Каждая операция имеет свой ЦКП
...
### 3. Pipeline = оркестратор операций
...
```python
# Можно тестировать каждую операцию отдельно
...
```python
# Добавить новую операцию = создать новый файл
# НЕ нужно трогать существующий код

# Новая операция: Denoiser
class ImageDenoiser:
...
src/extraction/
├── pre_ocr/
│   ├── elements/           # Отдельные операции (SRP)
...
│   │
│   ├── pipeline.py         # Оркестратор операций
...
| **ЦКП каждой операции** | Чёткий результат, который операция гарантирует |
| **Конфиг вместо хардкода** | Настройки вынесены, не зашиты в код |
| **Pipeline = оркестратор** | Сам не обрабатывает, только вызывает операции |
