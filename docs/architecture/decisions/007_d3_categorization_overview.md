...
## Открытый вопрос: Очистка `name`

**Статус:** К ОБСУЖДЕНИЮ

**Вопрос:** Нужно ли очищать `name` от:
- Налоговых классов в конце строки (A, B, и т.д.)?
- Сокращений магазина?
- Спецсимволов и знаков препинания?
- Кодов товаров и артикулов?

**Варианты:**
- **A:** Передавать как есть (LLM сам разберется)
- **B:** Очищать определенные элементы (какие именно?)
- **C:** Другой подход
...
2. **Задача D2**: передать `name` точно как на чеке, без потерь и искажений
...
5. **Не дублируем логику D3** в D2 (не очищаем, не переводим, не категоризируем)
