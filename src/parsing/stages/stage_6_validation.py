...
Входные данные: SemanticResult, MetadataResult
Выходные данные: ValidationResult (passed/failed, difference)
...
from .stage_5_semantic import SemanticResult
...
class ValidationStage:
    ...
    def process(
        ...
        Args:
            semantic: Результат Stage 5
            metadata: Результат Stage 4
            
        Returns:
            ValidationResult: Результат валидации
        """
        logger.debug("[Stage 6: Validation] Проверка checksum")
        ...
        if receipt_total is None:
            logger.warning("[Stage 6: Validation] Нет итога чека для валидации")
        ...
        if not passed:
            ...
            logger.error(f"[Stage 6: Validation] {error_message}")
        else:
            logger.info(
                f"[Stage 6: Validation] PASSED: {calculated_total} ≈ {receipt_total} "
