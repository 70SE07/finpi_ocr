...
from config.settings import ADDRESS_CONFIDENCE_HIGH
...
class AddressExtractor:
    ...
    Поддерживает LocaleConfig для расширения шаблонов.
    """

    # Шаблоны почтовых индексов (системный подход)
    ZIP_PATTERNS = {
        ...
        "TH": r"\b\d{5}\b",               # 10110 (Thailand)
        "TR": r"\b\d{5}\b",               # 34000 (Turkey)
        "KZ": r"\b\d{6}\b",               # 010000 (Kazakhstan)
    }
        ...
        for i, text in enumerate(texts[:15]):
            for country, pattern in self.ZIP_PATTERNS.items():
                if re.search(pattern, text):
                    ...
                    candidates.append(AddressResult(address=full_addr, confidence=ADDRESS_CONFIDENCE_HIGH, country_hint=country))
