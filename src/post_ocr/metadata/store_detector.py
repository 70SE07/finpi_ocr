...
class StoreDetector:
    ...
    # База брендов (может быть расширена через конфиг)
    BRANDS = {
    ...
    def detect(self, texts: List[str], address_res: Optional[AddressResult] = None) -> StoreResult:
        """
        ...
        header_text = "\n".join(texts[:STORE_SCAN_LIMIT]).lower()
        
        for brand, patterns in self.BRANDS.items():
        ...
        for brand, patterns in self.BRANDS.items():
            for pattern in patterns:
                if re.search(pattern, full_text, re.IGNORECASE):
                    ...
                    return StoreResult(name=brand, brand=brand, confidence=0.85)
        ...
        for text in texts[:STORE_FALLBACK_LIMIT]:
            ...
            if (len(clean_text) > MIN_STORE_NAME_LENGTH and 
